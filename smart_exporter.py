...
def generate_prometheus_metrics(
    ...
    for disk in disks:
        if disk.temperature > 0:
            metrics.append(
                f'snapraid_disk_temperature_celsius{{device="{disk.device}",serial="{disk.serial}",name="{disk.name}"}} {disk.temperature}'
            )
    ...
    for disk in disks:
        if disk.power_days > 0:
            metrics.append(
                f'snapraid_disk_power_on_days{{device="{disk.device}",serial="{disk.serial}",name="{disk.name}"}} {disk.power_days}'
            )
    ...
    for disk in disks:
        metrics.append(
            f'snapraid_disk_error_count{{device="{disk.device}",serial="{disk.serial}",name="{disk.name}"}} {disk.error_count}'
        )
    ...
    for disk in disks:
        metrics.append(
            f'snapraid_disk_failure_probability_percent{{device="{disk.device}",serial="{disk.serial}",name="{disk.name}"}} {disk.failure_prob}'
        )
    ...
    for disk in disks:
        ...
        metrics.append(
            f'snapraid_disk_size_bytes{{device="{disk.device}",serial="{disk.serial}",name="{disk.name}"}} {size_bytes}'
        )
