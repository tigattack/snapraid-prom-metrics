...
def parse_status_flags(output: str) -> StatusMetrics:
    ...
    return StatusMetrics(
        ...
        zero_subsecond_timestamps="No file has a zero sub-second timestamp"
        not in output,
...
def generate_prometheus_metrics(
    ...
    for disk in disks:
        metrics.append(
            f'snapraid_disk_fragmented_files{{name="{disk.name}"}} {disk.fragmented}'
        )
    ...
    for disk in disks:
        metrics.append(
            f'snapraid_disk_excess_fragments{{name="{disk.name}"}} {disk.excess}'
        )
    ...
    metrics.extend(
        [
            ...
            f"snapraid_array_files_total {array.total_files}",
            ...
            f"snapraid_array_fragmented_files_total {array.total_fragmented}",
            ...
            f"snapraid_array_excess_fragments_total {array.total_excess}",
...
def main():
    ...
    # Parse the output
    disks, array_totals, scrub_metrics, status_metrics = parse_snapraid_output(
        snapraid_output
    )

    # Generate metrics
    metrics = generate_prometheus_metrics(
        disks, array_totals, scrub_metrics, status_metrics
    )
